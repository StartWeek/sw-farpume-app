...
class MasterController extends Controller
{
    ...
    public function index(string $resource): Response
    {
        ...
        $rows = $model::query()
            ->when($resource === 'botol-kosung', fn ($query) => $query->with('gudang'))
            ->when($resource === 'supplier', fn ($query) => $query->with('gudang'))
        ...
        if ($resource === 'supplier') {
            $rows->setCollection($rows->getCollection()->map(function ($row) {
                $gudang = $row->relationLoaded('gudang') ? $row->gudang : $row->gudang()->first();
                $row->setAttribute('gudang_label', $gudang ? sprintf('%s - %s', $gudang->kode_gudang, $gudang->nama_gudang) : null);
                $row->setAttribute('gudang_type_label', $row->tipe_gudang ?: $gudang?->tipe_gudang);
    ...
    public function store(Request $request, string $resource): RedirectResponse
    {
        ...
        if ($resource === 'supplier' && ! Schema::hasColumn('tm_supplier', 'id_gudang')) {
            $payload = array_diff_key($payload, ['id_gudang' => true]);
        }

        if ($resource === 'supplier' && ! Schema::hasColumn('tm_supplier', 'tipe_gudang')) {
            $payload = array_diff_key($payload, ['tipe_gudang' => true]);
        }
    ...
    private function rules(string $resource): array
    {
        return match ($resource) {
            ...
            'gudang' => ['nama_gudang' => ['required', 'string', 'max:255'], 'tipe_gudang' => ['required', 'in:BIBIT,BOTOL'], 'alamat' => ['nullable', 'string'], 'status' => ['required', 'in:AKTIF,NONAKTIF']],
            'supplier' => ['nama_supplier' => ['required', 'string', 'max:255'], 'tipe_gudang' => ['nullable', 'in:BIBIT,BOTOL'], 'id_gudang' => ['nullable', 'exists:tm_gudang,id'], 'pic_name' => ['nullable', 'string', 'max:255'], 'no_hp' => ['nullable', 'string', 'max:50'], 'alamat' => ['nullable', 'string'], 'keterangan' => ['nullable', 'string'], 'status' => ['required', 'in:AKTIF,NONAKTIF']],
            ...
            'botol-kosong' => ['id_gudang' => ['required', 'exists:tm_gudang,id'], 'nama_botol' => ['required', 'string', 'max:255'], 'harga_beli' => ['nullable', 'numeric', 'min:0'], 'harga_jual' => ['nullable', 'numeric', 'min:0'], 'stock' => ['nullable', 'numeric', 'min:0'], 'satuan' => ['required', 'string', 'max:50'], 'status' => ['required', 'in:AKTIF,NONAKTIF'], 'keterangan' => ['nullable', 'string']],
            default => [],
        };
    }
    ...
    private function messages(string $resource): array
    {
        $labels = [
            ...
            'supplier' => ['nama_supplier' => 'Nama Supplier', 'tipe_gudang' => 'Tipe Gudang', 'id_gudang' => 'Gudang', 'pic_name' => 'PIC Name', 'no_hp' => 'No HP', 'alamat' => 'Alamat', 'keterangan' => 'Keterangan', 'status' => 'Status'],
            ...
            'botol-kosong' => ['id_gudang' => 'Kode Gudang', 'nama_botol' => 'Nama Botol', 'satuan' => 'Satuan', 'status' => 'Status'],
        ];
        ...
        if ($resource === 'supplier') {
            return [
                'gudang' => \App\Models\Business\Gudang::query()
                    ->where('status', 'AKTIF')
                    ->orderBy('kode_gudang')
                    ->get(['id', 'kode_gudang', 'nama_gudang', 'tipe_gudang']),
            ];
        }
    ...
    private function fields(string $resource): array
    {
        ...
        return match ($resource) {
            ...
            'gudang' => [['name' => 'nama_gudang', 'label' => 'Nama Gudang'], ['name' => 'tipe_gudang', 'label' => 'Tipe Gudang', 'type' => 'select', 'options' => ['BIBIT', 'BOTOL']], ['name' => 'alamat', 'label' => 'Alamat', 'type' => 'textarea'], $commonStatus],
            'supplier' => [['name' => 'nama_supplier', 'label' => 'Nama Supplier'], ['name' => 'tipe_gudang', 'label' => 'Tipe Gudang', 'type' => 'select', 'options' => ['BIBIT', 'BOTOL']], ['name' => 'id_gudang', 'label' => 'Gudang', 'type' => 'select', 'optionsRef' => 'gudang', 'optionValue' => 'id', 'optionLabel' => 'nama_gudang', 'optionDescription' => 'kode_gudang'], ['name' => 'pic_name', 'label' => 'PIC Name'], ['name' => 'no_hp', 'label' => 'No HP'], ['name' => 'alamat', 'label' => 'Alamat', 'type' => 'textarea'], ['name' => 'keterangan', 'label' => 'Keterangan', 'type' => 'textarea'], $commonStatus],
            ...
            'botol-kosong' => [['name' => 'id_gudang', 'label' => 'Kode Gudang', 'type' => 'select', 'optionsRef' => 'gudang', 'optionValue' => 'id', 'optionLabel' => 'kode_gudang', 'optionDescription' => 'nama_gudang'], ['name' => 'nama_botol', 'label' => 'Nama Botol'], ['name' => 'harga_beli', 'label' => 'Harga Beli', 'type' => 'number'], ['name' => 'harga_jual', 'label' => 'Harga Jual', 'type' => 'number'], ['name' => 'stock', 'label' => 'Stock', 'type' => 'number'], ['name' => 'satuan', 'label' => 'Satuan'], ['name' => 'keterangan', 'label' => 'Keterangan', 'type' => 'textarea'], $commonStatus],
            default => [],
        };
    }
}
